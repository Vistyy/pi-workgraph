...
Decode the strict current model policy: research and review are duplicate-free nonempty lists, while consultation is one target. An obsolete advisor array must fail with actionable conversion guidance before Task creation; do not retain an executable legacy shape or migration fixture. Inspection and control of already frozen Attempts must not depend on loading current policy.
