...
- `DESIGN.md` is repository-only and owns the integrated durable rationale. Read it before changing coordination workflow, tool interfaces, persistence, resource ownership, or architecture. It links to focused accepted decisions in `docs/adr/`; update the owning ADR instead of duplicating its trade-offs.
