...
Production names and owning documentation describe durable responsibilities, contracts, invariants, or safety facts, never delivery phases or change history. TypeScript source, tests, and scripts use no free-form `//` comments; only applicable rule-suppression directives and recognized top-of-file TypeScript compiler directives are allowed. Do not turn prose into block comments to evade this boundary.
