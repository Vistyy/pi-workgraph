...
Final local integration and publication use ordinary repository, forge, or session capabilities. The Coordinator contract owns the delivery-boundary trigger and explicit direct requests for guided review or pull-request follow or unfollow; the packaged delivery reference owns the Human sign-off requirement and handling, route recommendation and selection, and each selected route's procedure. A designated local session capability may support Maintainer inspection without becoming Workgraph state or authority. Neither the capability nor the procedure expands existing authority.

The optional Coordinator-only delivery loader reads the explicit `pi-workgraph.delivery.deferredTools` list from global settings. Workgraph has no peer defaults and imports no peer extension. A valid nonempty list removes only those names from the initial active set and keeps the loader active. Loading atomically adds every configured tool currently registered with Pi, preserves all active tools, and reports loaded, already-active, and missing names; visibility grants no authority and missing peers leave procedure fallbacks intact. Invalid settings fail open with a bounded warning.

A successful loader tool result is the branch-local visibility marker. Session start, reload, resume, fork, and tree navigation derive visibility from the current branch: descendants containing the marker inherit loaded peers, while navigation before it restores deferral. This uses Pi session history rather than Workgraph records. Peer lifecycle and background behavior remain independent of active visibility, and the loader never enters Worker scope or alters Worker `disabledTools` policy.
