...
Coordinator mutation tools return the exact persisted post-operation Attempt facts rather than a generic success flag. Exact Attempt inspection joins its immutable Task contract and target, semantic report status, Worker closure, repository disposition, bounded report preview, and exact runtime blocker without claiming acceptance. When an operation fails after a possible checkpoint or a multi-Attempt creation stops after earlier durable records, the error directs the Coordinator to inspect those facts before retrying; Workgraph adds no rollback or operation ledger.
