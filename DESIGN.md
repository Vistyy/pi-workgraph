...
Pi `sessionFile` is the durable Worker launch identity; Herdr owns current tab, pane, terminal, and agent truth. The coordinator creates and checkpoints the exact Attempt session before Herdr, and only that uninterrupted definitely-fresh path may submit one launch. Every later pass observes the owner workspace, exact session file, expected cwd, and bounded unique Attempt label. It adopts an exact ready Worker, but an absent resource, partial labelled tab, foreign identity, or ambiguity blocks without relaunching or automatically continuing a partial start.
Owned-worker cancellation is a separate durable boundary: the serialized call records the cancellation request, then may close the exactly observed ready or partial Worker once, and settles without a report only after exact absence is verified. Normal settlement similarly inserts the Outcome before its one close. Reload recovery is read-only: absence permits closure settlement, while continued presence, ambiguity, or foreign identity blocks and never repeats close. A crash after the durable request or Outcome but before close therefore requires deliberate human resource closure. Cancelled isolated output remains physically retained for explicit disposition, and graceful steering remains separate from termination.
