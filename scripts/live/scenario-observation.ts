...
/** The model-driven scenario keeps candidate application, correction, and review explicit. */
...
Next delegate disposable experiment id ${CAPABILITY_SCENARIO_IDS.uppercaseExperiment}, explicitly authorized to read value.txt and write only probe.txt with the exact bytes BEFORE followed by one newline (use a byte-preserving command such as printf 'BEFORE\\n' > probe.txt). Stop after one observation and retain probe.txt; never apply scratch code.
...
You may run read-only verification yourself but do not edit the fixture directly, and do not delegate extra workers or change model policy.
...
Verify before the correction application that the destination HEAD is unchanged at the fixture base, verify its application retains the complete initial-to-correction history, retain probe.txt with exact bytes BEFORE followed by one newline, and confirm all workers settle and every non-retained output is cleaned. Report a blocker instead of claiming success if any required boundary is unavailable.`;
}
