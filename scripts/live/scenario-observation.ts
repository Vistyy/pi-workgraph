...
Independently verify the retained experiment worktree contains probe.txt as exactly BEFORE followed by a newline, maintained value.txt is after followed by a newline, only value.txt changed, and node verify.mjs passes. Leave the retained experiment worktree in place until the live harness records those bytes; the harness will then issue one exact post-completion workgraph_control release_output action for that experiment attempt. Complete only after that release proves the exact worktree and branch are absent, all owned attempts are settled, and the native workspace is cleaned up.
