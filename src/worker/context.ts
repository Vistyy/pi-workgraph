...
const policies: Record<Exclude<WorkerRole, "implementation">, string> = {
  research: `[WORKGRAPH RESEARCH WORKER POLICY]\nAnswer the assigned question. The cwd is starting context, not an evidence boundary: inspect relevant accessible evidence beyond it when useful. ${readOnlyConduct} Context does not widen read-only authority. Details must give the answer, supporting and contrary evidence, and uncertainty. ${terminalStatusPolicy} Finish with workgraph_report.`,
