...
```bash
pi install npm:@syzom/pi-workgraph@0.3.0
```

Or install the matching GitHub release:

```bash
pi install git:github.com/Vistyy/pi-workgraph@v0.3.0
```
...
When an accepted change has no explicit delivery route, the Coordinator stops before those final effects, classifies whether Human sign-off is required, recommends pull-request delivery, local integration, or preserving the ready checkout, and waits for the user's choice. If pull-request delivery is selected, the Coordinator then reads its packaged publication reference for a concise visual explanation, claim-appropriate evidence, and one-time publication with the complete resulting URL; it does not merge or monitor the pull request.

A Task has one immutable resolved target. Directory targets record an exact path. Repository targets record the checkout root and Git common directory, so Tasks in one session may safely address different repositories without implying a transaction across them. Every Attempt inherits its Task target and starts one fresh Worker Pi session. `workgraph_attempt` creates another execution of that unchanged assignment; a changed question, request, context, acceptance condition, or authority requires a new Task.
