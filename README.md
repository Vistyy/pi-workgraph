...
- **Results are retained, not automatically applied.** The coordinator judges the evidence and explicitly selects the exact attempt to apply. The runtime derives its source/root and verifies the live destination from the retained branch. A retained candidate can be corrected with `workgraph_implement` and `candidateOf`, or explicitly integrated onto a moved destination with `candidateOf` and an exact current `baseRevision`; applying a correction fast-forwards its complete direct history without squashing or requiring a checkout.
