...
When an accepted change has no explicit delivery route, the Coordinator stops before those final effects, classifies whether human review is required, recommends pull-request delivery, local integration, or preserving the ready checkout, and waits for the user's choice. If pull-request delivery is selected, a packaged on-demand skill guides one-time publication with a concise visual explanation, claim-appropriate evidence, and the complete resulting URL; it does not merge or monitor the pull request.
