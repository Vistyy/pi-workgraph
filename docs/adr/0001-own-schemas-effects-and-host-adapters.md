...
Pi 0.84.4 defines tool parameters as TypeBox `TSchema` values and derives handler input through `Static<T>`. Workgraph's tool contracts and canonical persisted model also share report, evidence, model-target, and review-subject shapes. Effect Schema can emit JSON Schema, but making it the internal owner would leave TypeBox at the Pi boundary and introduce generated-schema typing, optional-property, strictness, and provider-compatibility obligations.
...
- Canonical and tool schemas can share definitions without translation or duplicate validation.
...
- The temporary duplication between v7 and canonical schema families is removed by deleting v7, not by introducing another schema library.
