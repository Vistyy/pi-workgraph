...
Persistence is one native SQLite file partitioned by exact Pi session. Strict tables store Tasks, Attempts, and the current Coordinator checkout lifecycle for each session and repository. Task creation with its first Attempt, focused operational checkpoints, checkout delivery checkpoints, and write-once Outcome insertion use narrow transactions. The store has no in-memory aggregate copy, revision protocol, or lifecycle authority outside those rows. [ADR 0004](0004-own-checkout-delivery.md) owns the checkout record and delivery decision.
