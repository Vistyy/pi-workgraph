...
Inspect the actual candidate and choose explicitly whether to apply or retain it; settlement never applies output. Select the exact attempt; application proves its source lineage, previews a clean attached destination and conflicts with Git off-checkout before checkpointing, then re-proves intent, ownership, candidate tip, and destination while preparing an already-integrated result, linear fast-forward, or fresh ordered-parent merge. Mutation uses only `git merge --ff-only`. `candidateOf` supports direct correction and explicit integration onto an exact current `baseRevision` when conflict, rewritten/unrelated history, or semantic integration needs a new candidate. Recovery is structural rather than causal: unchanged non-ancestral state remains retryable, exact candidate/merge shape may prove application, and ambiguity blocks without rollback or automatic retry.
