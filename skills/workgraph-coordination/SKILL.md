...
Workers start fresh. Use `continuationOf` only when a settled worker's retained session trajectory is useful. It is not content lineage: use `candidateOf` for a maintained candidate's direct correction or explicit integration onto an exact current `baseRevision`.

## Integrate and finish the agreed work

Inspect the actual candidate and choose explicitly whether to apply or retain it; settlement never applies output. Select the exact attempt; application proves its source lineage, previews a clean attached destination and conflicts with Git off-checkout before checkpointing, then re-proves intent, ownership, candidate tip, and destination while preparing an already-integrated result, linear fast-forward, or fresh ordered-parent merge. Mutation uses only `git merge --ff-only`, and both actions post-check the returned revision against the prepared clean attached ref. `candidateOf` supports direct correction and explicit integration onto an exact current `baseRevision` when conflict, rewritten/unrelated history, or semantic integration needs a new candidate. Recovery is structural rather than causal: unchanged non-ancestral state remains retryable, exact candidate/merge shape may prove application, and a switched ref or other ambiguity blocks without rollback or automatic retry; an applied checkpoint retry skips Git application and resumes exact output release.
